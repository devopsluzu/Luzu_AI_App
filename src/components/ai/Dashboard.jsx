...
const AiDashboard = ({ menuOpen, setMenuOpen }) => {
  ...
  return (
    <div className={styles.aiLeftDashboard} ref={dashboardRef} style={dashboardHeight ? { height: dashboardHeight } : {}}>
  <div className={styles.aiLeftDashboardContainer}>
    ...
    <div className={styles.aiLeftDashboardAgents}>
      <h2>Agents</h2>
      <Link href="/content-generation" className={`${styles.aiLeftDashboardContents} ${pathname.startsWith('/content-generation') ? styles.active : ''}`}>
        Content Generation
      </Link>
      <Link href="/keyword" className={`${styles.aiLeftDashboardContents} ${pathname.startsWith('/keyword') ? styles.active : ''}`}>
        Keyword Research
      </Link>
      <Link href="/competitor" className={`${styles.aiLeftDashboardContents} ${pathname.startsWith('/competitor') ? styles.active : ''}`}>
        Competitor Analysis
      </Link>
    </div>
      ...
      <div className={styles.chatDashboardRecentsContents}>
        {pathname.startsWith('/content-generation') && recentPrompts.length > 0 && (
          <>
            ...
            {visiblePrompts < recentPrompts.length && (
              <div className={styles.chatDashboardViewMoreBtn} onClick={() => setVisiblePrompts(prev => prev + INITIAL_VISIBLE_CHATS)}>
                View More
              </div>
            )}
          </>
          ...
          <>
            {recentKeywords.slice(0, visibleKeywords).map((keyword) => (
              <Link 
                ...
                className={`${styles.contentGenerationRecentSearch} ${pathname.startsWith(`/keyword/${keyword.keywordId}`) ? styles.active : ''}`}
              >
                {keyword.id}
              </Link>
            ))}
            {visibleKeywords < recentKeywords.length && (
              <div className={styles.chatDashboardViewMoreBtn} onClick={() => setVisibleKeywords(prev => prev + INITIAL_VISIBLE_CHATS)}>
                View More
              </div>
            )}
          </>
          ...
          <>
            {recentCompetitor.slice(0, visibleCompetitor).map((competitor) => (
              <Link 
                ...
                className={`${styles.contentGenerationRecentSearch} ${pathname.startsWith(`/competitor/${competitor.competitorId}`) ? styles.active : ''}`}
              >
                {competitor.id.replace(/_/g, ".")}
              </Link>
            ))}
            {visibleCompetitor < recentCompetitor.length && (
              <div className={styles.chatDashboardViewMoreBtn} onClick={() => setVisibleCompetitor(prev => prev + INITIAL_VISIBLE_CHATS)}>
                View More
              </div>
            )}
          </>
        ...
        {(pathname === '/' || pathname.startsWith('/chat')) && recentChats.length > 0 && (
          <>
            ...
            {visibleChats < recentChats.length && (
              <div className={styles.chatDashboardViewMoreBtn} onClick={() => setVisibleChats(prev => prev + INITIAL_VISIBLE_CHATS)}>
                View More
              </div>
            )}
          </>
      ...
      {openSetting && (
        <div className={styles.chatDashboardSettings}>
          <div className={styles.chatDashboardSettingsContainer}>
            <div className={styles.chatDashboardSettingsTheme} onClick={toggleTheme}>
              <p>Theme</p>
              <div className={styles.settingsThemeIndicator}>
                <span className={`${styles.settingsThemeIndicatorLight} ${currentTheme === 'light' ? styles.active : ''}`}></span>
                <span className={`${styles.settingsThemeIndicatorDark} ${currentTheme === 'dark' ? styles.active : ''}`}></span>
              </div>
            </div>
            <div className={styles.chatDashboardSettingsLogout} onClick={logOut}>
              Logout
            </div>
          </div>
        </div>
      )}
      <div className={styles.chatDashboardAccountContainer} onClick={() => setOpenSetting(!openSetting)}>
        <div className={styles.chatDashboardAccountContents}>
          <div className={styles.chatDashboardAccountEmail}>
            Settings
          </div>
          <IoSettingsOutline style={{ fontSize: "16px", color: "var(--p-color)" }} />
        </div>
      </div>
    </div>
