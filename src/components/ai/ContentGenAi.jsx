...
<div className={`${styles.luzuAi} luzu-ai`}>
      <div className={styles.luzuAiContainer}>
        ...
        <div className={styles.contentGenSpace}>
          <div className={`${styles.chatSpace} ${!messages.some((msg) => msg.sender === 'AI') && !isTyping ? styles.centerContent : ""}`}>
            {!messages.some((msg) => msg.sender === 'AI') && !isTyping && (
              <h2 className={styles.emptyStateTitle}>Generate SEO Ready Blogs</h2>
            )}
                ...
                {messages
                  ...
                  .map((msg, index) => (
                    <div key={index} className={styles.aiMessage}>
                      {formattedTitle && (
                        <h1 className={styles.heading1} dangerouslySetInnerHTML={{ __html: formattedTitle }} />
                      )}
                      {metaDescription && (
                        <p className={styles.metaDescription} dangerouslySetInnerHTML={{ __html: metaDescription }} />
                      )}
                      {formattedContent && (
